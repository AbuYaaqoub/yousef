...
const EXCEL_FILE_PATH = path.join(process.cwd(), 'Mazeed_Leads.xlsx');

export async function saveToMazeedExcel(productName: string, data: any[]) {
    ...
    try {
        ...
        console.log(`✅ Excel Updated: Added ${addedCount} new stores to Mazeed sheet [${sheetName}]`);
    } catch (err: any) {
        if (err.code === 'EBUSY') {
            console.error('❌ Error: The Excel file is currently open. Please close "Mazeed_Leads.xlsx" and try again.');
...
export async function mergeMazeedSheets(sourceSheetNames: string[], targetSheetName: string) {
    if (!fs.existsSync(EXCEL_FILE_PATH)) {
        throw new Error('ملف بيانات مزيد غير موجود بعد.');
    }
    ...
    try {
        ...
        return {
            ...
            message: `تم دمج ${validSheets.length} أوراق وإضافة ${uniqueStores.size} متجر فريد في الورقة "${safeTargetName}"`
        };
    } catch (err: any) {
        if (err.code === 'EBUSY') {
            throw new Error('الملف مفتوح حالياً في برنامج آخر، يرجى إغلاقه والمحاولة مجدداً.');
